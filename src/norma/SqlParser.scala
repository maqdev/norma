...
object SqlParser {
  ...
  def flatten[T1, T2, R](implicit f: norma.TupleFlattener[(T1 ~ T2) => R]): ((T1 ~ T2) => R) = f.f

  def str(columnName: String): RowParser[String] = get[String](columnName)(implicitly[norma.Column[String]])
  ...
  def getAliased[T](aliasName: String)(implicit extractor: norma.Column[T]): RowParser[T] = RowParser { row =>
    import MayErr._
  ...
  def get[T](columnName: String)(implicit extractor: norma.Column[T]): RowParser[T] = RowParser { row =>
    import MayErr._
